...
class Climb(Base):
    ...
    def init_conditions(self, **kwargs):
        ...
        if h_end is None:
            h_end = self.aircraft["cruise"]["height"]
        ...
        self.u_guess = [0.3, 1500 * fpm, psi_end]
    ...
    def trajectory(self, objective="fuel", df_cruise=None, **kwargs) -> pd.DataFrame:
        ...
        for k in range(1, self.nodes):
            ...
            lbg.append([-12 * fpm])  # per second
            ubg.append([12 * fpm])  # per second
        ...
        for k in range(self.nodes):
            ...
            cas_max = self.aircraft["vmo"] * kts * 1
            g.append(cas)
            lbg.append([0])
            ubg.append([cas_max])
